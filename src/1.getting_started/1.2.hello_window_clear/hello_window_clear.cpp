/***********************************************************
 ...
 Filename     : hello_window_clear.cpp
 Description  : 通过 6 大步创建一个窗口, 并在第五步的时候重置其颜色
                1. 初始化与配置 glfw
                2. 创建 glfw 窗口
                3. 设置渲染窗口大小
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //5.2 渲染操作
        glClearColor(0.2f, 0.3f, 0.3f, 1.0f);//5.2.1 设置颜色
        glClear(GL_COLOR_BUFFER_BIT);//5.2.2 将制定的缓冲区都填充为上面的颜色 
        //参数可以使 GL_COLOR_BUFFER_BITJK, GL_DEPTH_BUFFER_BIT, GL_STENCIL_BUFFER_BIT
...
//    glfwSetFramebufferSizeCallback(window, framebuffer_size_callback);//将 <窗口指针> 与 <函数指针> 传给该函数
//
