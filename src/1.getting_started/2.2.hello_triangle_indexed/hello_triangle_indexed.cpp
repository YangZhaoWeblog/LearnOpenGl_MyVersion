...
int main()
{
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //glDrawArrays(GL_TRIANGLES, 0, 6);//不用 vao 方式绘制
        glDrawElements(GL_TRIANGLES, 6, GL_UNSIGNED_INT, 0);//而是使用 EBO 形式绘制
        glBindVertexArray(0);//解绑 VAO 对象
...
void init_set_VAO_VBO_EBO(GLuint& VAO, GLuint& VBO, GLuint& EBO)
{
  ...
  float vertices[] = {
    ...
    //4. 为 EBO 对象设置状态值, 具体意义为: 选取哪 EBO 几个坐标, 构建三角形
