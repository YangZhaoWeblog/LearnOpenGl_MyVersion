...
int main()
{
    ...
    //2. 配置着色器
    Joe::JoeShader ourShader("5.2.transform.vs", "5.2.transform.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        //glBindVertexArray(0); //这行代码千万不能用, 因为后面一次绘制还需要用呢！

        //----渲染第二个图形---
        transform = glm::mat4(1.0f); // reset it to identity matrix
        transform = glm::translate(transform, glm::vec3(-0.5f, 0.5f, 0.0f));
        float scaleAmount = static_cast<float>(sin(glfwGetTime()));
        transform = glm::scale(transform, glm::vec3(scaleAmount, scaleAmount, scaleAmount));//这个 vector 中的三个坐标不断的缩放
        //需要 矩阵transform 的首地址
